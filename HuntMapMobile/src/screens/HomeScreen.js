...
// Mock hunts — replaced with real API data in Phase 2
const MOCK_HUNTS = [
...
export default function HomeScreen() {
  ...
  const filteredHunts = MOCK_HUNTS.filter(h => {
    ...
    const matchesFilter =
      ...
      ['Nearby', 'Popular', 'New'].includes(activeFilter); // placeholders for now
    return matchesSearch && matchesFilter;
  });

  if (Platform.OS === 'web') {
    return <WebDiscover colors={colors} s={s} location={location} loading={loading} search={search} setSearch={setSearch} activeFilter={activeFilter} setActiveFilter={setActiveFilter} filteredHunts={filteredHunts} />;
  }

  return <MobileDiscover colors={colors} s={s} location={location} loading={loading} search={search} setSearch={setSearch} filteredHunts={filteredHunts} />;
}

// ── Web layout: map left 60%, feed right 40% ─────────────────────────────────

function WebDiscover({ colors, s, location, loading, search, setSearch, activeFilter, setActiveFilter, filteredHunts }) {
  return (
